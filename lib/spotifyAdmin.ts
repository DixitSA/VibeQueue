...
// Reads/writes venue_settings/{venueId} for token management.
...
async function getAccessToken(venueId: string): Promise<string> {
  const snap = await getDoc(doc(db, 'venue_settings', venueId));
  if (!snap.exists()) throw new Error(`[VibeQueue] No settings for venue: ${venueId}`);

  const data = snap.data();
  if (!data.spotifyAccessToken) throw new Error('[VibeQueue] Spotify not connected for this venue.');

  // Use cached token if it has > 60 s remaining
  const expiresAt: number = data.tokenExpiresAt?.toDate?.()?.getTime() ?? 0;
...
async function refreshAccessToken(venueId: string, refreshToken: string): Promise<string> {
  ...
  // Persist refreshed token (Spotify may also rotate the refresh token)
